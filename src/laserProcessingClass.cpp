...
void LaserProcessingClass::featureExtraction(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_edge, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_surf){
    ...
    double startOri = -atan2(pc_in->points[0].y, pc_in->points[0].x);
    double endOri = -atan2(pc_in->points[cloudSize-1].y, pc_in->points[cloudSize-1].x) + 2*M_PI;
    ...
    for (int i = 0; i < (int) pc_in->points.size(); i++)
    {
        ...
        /*******************************************************************************/
        // 计算每个点的time信息
        double ori = -atan2(pc_in->points[i].y, pc_in->points[i].x);
        if (!halfPassed)
        {
