//
// Created by ywl on 22-11-14.
//

#ifndef MCT_LOAM_LIDARFACTOR_HPP
#define MCT_LOAM_LIDARFACTOR_HPP
...
struct LidarFirstPlaneFactor
{
    ...
    {
        // 自动求导模板
        return (new ceres::AutoDiffCostFunction<LidarFirstPlaneFactor, 1, 4, 3>(new LidarFirstPlaneFactor(curr_point_, plane_unit_norm_,
...
struct LidarSecondPlaneFactor
{
    ...
    template <typename T>
    bool operator()(const T* parameters_middle_q, const T* parameters_middle_t, const T* parameters_end_q, const T* parameters_end_t, T* residual) const
    {
        // 优化变量，当前帧中间时刻和结束时刻的位姿
        Eigen::Quaternion<T> q_w_curr_middle{parameters_middle_q[3], parameters_middle_q[0], parameters_middle_q[1], parameters_middle_q[2]};
        Eigen::Matrix<T, 3, 1> t_w_curr_middle{parameters_middle_t[0], parameters_middle_t[1], parameters_middle_t[2]};
    ...
    {
        return (new ceres::AutoDiffCostFunction<LidarSecondPlaneFactor, 1, 4, 3, 4, 3>(new LidarSecondPlaneFactor(curr_point_, plane_unit_norm_, negative_OA_dot_norm_, time_)));
    }
...
struct VelocityConsistentyFactor
{
    ...
    bool operator()(const T* parameters_middle_q, const T* parameters_middle_t, const T* parameters_end_q, const T* parameters_end_t, T* residual) const
    {
        ...
        residual[0] = delta_t.x() * factor;
        residual[1] = delta_t.y() * factor;
        residual[2] = delta_t.z() * factor;

        residual[3] = delta_q.x() * factor;
        residual[4] = delta_q.y() * factor;
        residual[5] = delta_q.z() * factor;
        residual[6] = (delta_q.w() - T(1)) * factor;
    ...
    {
        return (new ceres::AutoDiffCostFunction<VelocityConsistentyFactor, 7, 4, 3, 4, 3>(new VelocityConsistentyFactor(q_w_last_end_,
                                                                                                                  t_w_last_end)));
    }
...
#endif //MCT_LOAM_LIDARFACTOR_HPP
